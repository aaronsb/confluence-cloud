...
async function handleDelete(args: WorkspaceArgs): Promise<ToolResponse> {
  ...
  try {
    ...
    if (stat.isDirectory()) {
      ...
      return { content: [{ type: 'text', text: `Deleted directory: ${args.filename}` }] };
    }
...
async function handleMkdir(args: WorkspaceArgs): Promise<ToolResponse> {
  if (!args.filename) {
...
async function handleMove(args: WorkspaceArgs): Promise<ToolResponse> {
  if (!args.filename) {
